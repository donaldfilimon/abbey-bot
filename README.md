...
| `/voice consent`; `/voice notice`; `/voice join consent:true\|resume consent:true\|leave\|status\|mode`; `/voice verify start\|report` | `/voice consent` lets each member review, agree or withdraw privately. A manager can publish the same controls with `/voice notice`. `/voice mode` shows the backend in force and switches among the backends configured at startup while no session is open; it takes effect on the next join. Join/resume require Manage Server, the caller to be present, saved individual agreement from every current participant for the selected processing mode, and public disclosure before the media gate opens. New or unidentified participants stop and disconnect the call. Leave is available to someone present or a manager. The owner/admin-only local verifier uses content-free in-memory counters and disables voice conversation commits while armed. Automatic presence is muted and self-deafened. |
...
media gate and tears down the conversational connection. Typing `stop listening`
in the configured voice chat does the same, including while already paused.
