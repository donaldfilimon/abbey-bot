...
|-------|--------|----------|
| Source gate (`./check.sh`) | **PASS** (Rust + strict WDBX re-measured 2026-09-16 on `4bd8dbf`; macOS only) | **1,294 Rust tests passed, 0 failed, 5 intentional live ignores**; warnings-denied all-target locked clippy and the locked release build passed; strict sibling WDBX conformance passed against `../wdbx` at `2cf550c`. The Swift and audio-tap sub-counts previously quoted here (27 / 25) are **from an earlier run and were not re-measured** — that stage was not reached in the 2026-09-16 pass. See [catalog/audio-tap source evidence](docs/superpowers/plans/2026-09-04-catalog-audio-tap-evidence.md). |
...
- **WDBX episode gateway (this Mac)** — `deploy/install-wdbx-gateway-launchd.sh
  ...
  `deploy/com.donaldfilimon.abbey-wdbx-gateway.plist` (loopback
