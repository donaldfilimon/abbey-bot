...
See `docs/benchmarks/2026-08-19-local-models.md`. Result: **gpt-oss:20b** (7–25 s, 124–3069 chars of reasoning, native tool calling) is the default; gemma4:e4b is the runner-up (13–37 s, best register); gemma4:12b / qwen3.5 / ornith:9b are reasoning runaways (47–182 s).
...
4. **Reasoning stays in the model's head — by model choice, not by flag.** ollama's `/v1` ignored `think` and returned empty bodies for `reasoning_effort` in today's probe, so no reasoning control is sent; the benchmark picked a model that reasons briefly instead. `extract_text` already reports reasoning-only bodies honestly.
