...
pub fn play(player: Player, query: &str) -> Result<Script, &'static str> {
    ...
    if query.len() > 500 || query.chars().any(char::is_control) {
        return Err("Use a query under 500 bytes without control characters.");
    }
    let source = match player {
        Player::Spotify => {
            if !query.is_empty()
                && !(query.strip_prefix("spotify:track:").is_some_and(|id| {
                    id.len() == 22 && id.bytes().all(|b| b.is_ascii_alphanumeric())
                }))
            {
                return Err(
                    "Spotify accepts a spotify:track: URI, or an empty query to mirror the current selection. Use Music for library text search.",
                );
            }
            "on run argv\ntell application id \"com.spotify.client\"\nif item 1 of argv is \"\" then\nplay\nelse\nplay track (item 1 of argv)\nend if\nend tell\nend run"
        }
        Player::Music => {
            "on run argv\ntell application id \"com.apple.Music\"\nif item 1 of argv is \"\" then\nplay\nelse\nset matches to search library playlist 1 for (item 1 of argv) only names\nif (count of matches) is 0 then error \"No matching library track\"\nplay item 1 of matches\nend if\nend tell\nend run"
        }
    };
    Ok(Script {
        source,
...
mod tests {
    use super::*;
    #[test]
    fn music_query_cannot_become_code() {
        let query = "\" & do shell script \"touch /tmp/no\"";
        let script = play(Player::Music, query).unwrap();
        assert!(!script.source.contains(query));
        assert_eq!(script.argument, query);
        assert!(
            script
                .source
    ...
    #[test]
    fn spotify_and_pause_golden() {
        assert_eq!(
            pause(Player::Spotify).source,
            "on run argv\ntell application id \"com.spotify.client\" to pause\nend run"
        );
        assert_eq!(
            pause(Player::Music).source,
            "on run argv\ntell application id \"com.apple.Music\" to pause\nend run"
        );
        ...
        for query in [
            ...
            assert!(play(Player::Spotify, query).is_err());
        }
