//! Zig-compatible Wyhash — a transcription of abi's port, not a dependency.
//!
//! This is a faithful port of `std.hash.Wyhash` from the Zig toolchain abi pins,
//! carried over line-for-line from `abi-foundation/src/wyhash.rs`. It exists
//! because **no Rust crate matches it**: the `wyhash` 0.5 crate implements a
//! different version of the algorithm and returns entirely different values —
//! for seed `3` over `"hel"`, Zig gives `10846395113768030678` and the crate
//! gives `490820195397404894`.
//!
//! That distinction is load-bearing rather than academic. [`crate::embedding`]
//! derives each of its 32 vector buckets *and each bucket's sign* from this
//! hash, and the resulting vectors are persisted into a WDBX store that abi can
//! also read. A different hash would produce embeddings that do not match abi's,
//! so cosine and semantic search across the two would be silently wrong — no
//! error, just degraded ranking. abbey-bot takes no path or git dependency on
//! abi by design, so the algorithm is transcribed here and pinned by abi's own
//! Zig-emitted reference vectors (`tests/fixtures/wyhash_zig_refs.txt`).
...
/// Zig-compatible Wyhash state.
///
/// Callers want [`hash`]. The struct mirrors Zig's `init`/`update`/`final`
/// shape so the two implementations diff cleanly, and keeps one subtlety in
/// one place: an input that is exactly 48-byte aligned must run its last full
...
mod tests {
    ...
    /// The `as u8` is the deliberate low-byte truncation (= Zig `@truncate`).
    ...
    fn matches_every_zig_reference_vector() {
        let refs = include_str!("../tests/fixtures/wyhash_zig_refs.txt");
        let mut checked = 0;
    ...
    fn matches_zig_on_the_embedding_gram_shapes() {
        // The exact call shape text_embedding uses: seed = n-gram length, input =
        // the lowercased gram. These are the values a wrong Wyhash would
    ...
    fn differs_from_the_rust_wyhash_crate() {
        // Guards the whole reason this module exists.
        assert_ne!(hash(3, b"hel"), 490_820_195_397_404_894);
    }

    #[test]
    fn seed_changes_the_digest() {
        assert_ne!(hash(0, b"abbey"), hash(1, b"abbey"));
    }
