...
- **2026-09-19 correction (ledger restoration for #170/#171/#172 + direct merges):** restores the substance of the 2026-09-17 05:1x EDT line removed by `e6abb8a` (append-only; history untouched): #170 (`7564cfc`, `/roleplay` NSFW gate — bot DM or NSFW guild channel + durable nsfw flag, fail-closed) → #171 (`c6ddb02`, roleplay persona dead-code fix — callers take both `RoleplayDecision::allow()` and `persona()`, `14ba54e` takes persona from the gate) → #172 (`bae9214`, roleplay-guidance docs — `roleplay_gate.rs` boundary in AGENTS.md/CLAUDE.md, reviewer restatements name `./check.sh`, `51c3c40`), then two direct merges atop #172: `e6abb8a` (ask-dejargon + ask-prompt worktree content) and tip `1864796`. Verified against `git log --oneline -8 origin/main` and `git show e6abb8a -- tasks/goals.md`. The out-of-order 2026-09-16 de-jargon line stays where it is (append-only).
